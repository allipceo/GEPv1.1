...
**개정:** 2026-08-17 (v1.1 — GEPv30-122 카운팅가드 검증·GEPv30-123 노팀장 검토소견 결정 C 반영, 4.3절 7항 추가)
...
7. **(v1.1 추가)** 같은 세션 내에서 이전 화면으로 돌아가 답을 바꿔 재제출한 경우, 화면 표시는 마지막 제출값으로 갱신되나 attempts 테이블 집계는 최초 제출값 1건만 유지된다. (GEPv30-122 카운팅가드 검증 결과 반영 — 버그가 아니라 5항의 자연스러운 귀결이며, 혼란 방지를 위해 명문화한다)
