...
## 12. 추가 수정 4 — OX 일반 풀기(이어풀기)가 항상 처음부터 시작하던 문제 (2026-08-20)

이번엔 GEPv30-138(틀린문제 재도전)이 아니라 **원래 있던 OX 일반 풀기 화면**(`/ox/:subjectKey/:subSubject`, Phase 4)에서 조대표가 "누적 37문항인데 세부과목을 다시 선택하면 38번부터가 아니라 1번부터 다시 시작한다"고 제보했다.

### 12-1. 원인

`oxService.js`에 `saveProgress`/`loadProgress` 함수가 이미 존재했고 `progress` 테이블에 `current_index` 컬럼도 있었지만, **`loadProgress`를 호출하는 코드가 프로젝트 전체에 단 한 곳도 없었다**(grep으로 전수 확인). 즉 "이어풀기" 기능은 저장(`saveProgress`)만 하고 읽어오는(`loadProgress`) 코드가 애초에 연결되지 않은 죽은 코드였다 — Phase 4(GEP_042/043) 때부터 있던 구조적 공백으로, 이번 GEPv30-138 작업과는 무관한 기존 결함이다.

게다가 기존 `saveProgress` 호출(`completeRound()`)은 **360문항 전체를 한 번에 다 풀어야만** 실행되는 구조라, 중간에 나가면(이번 테스트처럼 37/360) 애초에 저장 자체가 안 되고 있었다. 실측: 테스터3 계정의 `progress` 테이블에 `ox:` 로 시작하는 행이 **0건** — 저장도, 조회도 실제로는 한 번도 일어난 적이 없었음을 확인했다.

### 12-2. 조치

| 파일 | 조치 |
|---|---|
| `src/services/oxService.js` | `current_index`의 의미를 "라운드 번호"에서 "재개할 문항 인덱스"로 명확히 재정의(로그인 기록엔 영향 없음, 두 값 모두 실제로는 소비된 적 없는 필드였음) |
| `src/stores/oxStore.js` | `goNext()`에서 문항을 넘길 때마다 현재 인덱스를 `saveProgress`로 저장(모아풀기 모드는 별도 부분집합이라 제외), `completeRound()`(라운드 완주)에서는 인덱스를 0으로 리셋 저장, `loadQuestions()`가 `startIndex` 파라미터를 받아 그 위치에서 시작하도록 확장 |
| `src/pages/OXSubject.jsx` | 세부과목 카드 클릭 시 `loadProgress()`로 저장된 재개 지점을 조회해 `loadQuestions()`에 전달하도록 연결(이 연결이 누락되어 있던 것이 근본 원인) |

### 12-3. 검증

`npm run build` 재확인 성공. 이 결함은 이번 GEPv30-138 세션에서 처음 저장이 시작되므로, **조대표가 이후 몇 문제를 새로 풀고 다른 화면으로 나갔다가 같은 세부과목으로 재진입**해야 재개 지점 저장 효과를 확인할 수 있다(과거 37문항 풀이 기록에는 재개 지점이 애초에 저장되지 않았으므로 소급 적용되지 않음).
