...
### 철학 6: UI 목업 우선 개발 프로세스 ★ (2026-08-08 조대표님 확정)

> **"메뉴트리 도식화 → 화면별 목업 → 대표님 승인 → 개발"** 이 순서는 모든 신규 기능 개발에 고정 적용한다.

#### 단계별 프로세스
```
STEP 1. 메뉴트리 도식화 (노팀장)
  - UI 레이어 트리 작성 (L[레이어]-[경로] ID 체계)
  - 각 화면 간 이동 경로 시각화
  - 대표님 승인 게이트

STEP 2. 화면별 목업 (노팀장)
  - 모바일 기준 실제 화면 비율로 작성
  - 각 UI 요소에 ID 표기 (L1-DB, L2-A 등)
  - 고팀장 작업 범위 명시
  - 대표님 승인 게이트
...
STEP 4. 개발 실행 (고팀장)
  - 지시서 범위 내 파일만 수정
  - 빌드 성공 확인
  - 문서화(GEPv30-XXX) 후 git push

STEP 5. 검증 (노팀장 1차 → 대표님 최종)
  - 목업과 실제 화면 대조 검증
  - 노션 업데이트
```
...
| 역할 | 노팀장 | 고팀장 |
|------|--------|--------|
| 강점 | 기획·설계·목업·문서화·승인관리 | 코딩·빌드·배포·디버깅 |
| 산출물 | 트리도식·목업·지시서·노션문서 | 소스코드·빌드결과·GEPv30-XXX |
| 금지 | 코딩 직접 실행 | 지시 범위 외 파일 수정 |
