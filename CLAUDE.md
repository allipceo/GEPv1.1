...
2. Git 배포:
   ```bash
   git add .
   git commit -m "GEP_XXX: 작업명"
   git push origin main
   ```
   - ⚠️ `vercel --prod` 직접 실행 금지
   - GitHub push → Vercel 자동 배포
3. 작업 문서 생성 (docs/GEP_XXX_작업명.md)
4. 노팀장 창에 보고
...
### 규칙
**파일 1개 이상 수정하는 모든 작업 완료 시** 아래 규칙으로 작업 문서를 **반드시** 생성한다.
...
- **현재 최신:** GEP_064 (다음 작업은 GEP_065부터 시작)
